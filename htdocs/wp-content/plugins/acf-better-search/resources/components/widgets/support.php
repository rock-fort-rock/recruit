<div class="acfbsPage__widget">
  <h3 class="acfbsPage__widgetTitle acfbsPage__widgetTitle--second">
    <?= __('We are waiting for your message', 'acf-better-search'); ?>
  </h3>
  <div class="acfbsContent">
    <p>
      <?= __('Do you have a technical problem? Please contact us. We will be happy to help you. Or maybe you have an idea for a new feature? Please let us know about it by filling the support form. We will try to add it!', 'acf-better-search'); ?>
    </p>
    <p>
      <a href="https://wordpress.org/support/plugin/acf-better-search/#new-post" target="_blank" class="acfbsButton acfbsButton--blue">
        <?= __('Get help', 'acf-better-search'); ?>
      </a>
    </p>
    <p>
      <?= __('Do you like our plugin? Could you rate him? Please let us know what you think about our plugin. It is important that we can develop this tool. Thank you for all the ratings, reviews and donates.', 'acf-better-search'); ?>
    </p>
    <p>
      <a href="https://wordpress.org/support/plugin/acf-better-search/reviews/#new-post" target="_blank" class="acfbsButton acfbsButton--blue">
        <?= __('Add review', 'acf-better-search'); ?>
      </a>
    </p>
  </div>
</div>